...
//------------------------------------end 删除注册设备 --------------------------------------
  ...
  render() {
      ...
      },
      {
        ...
        formatter: ({dependentValues, rowIdx, value, column: {key}})=>{
          return (
            <div>
              <div>
                {dependentValues.deviceTypeId === 10 ?
                  <span onClick={this.viewPOCTBoxClick.bind(this, {rowdata: dependentValues})}><a>查看子模块</a>
                    <Divider type="vertical" />（未解决poctBox类型识别）</span>
                  :
                  ''
                }
    ...
    return (
      <PageHeaderLayout>
        <div className="yzh-d-remained-submit-approval has-no-check-box-all">
...
DeviceRegister.defaultProps = {
  prefix: 'yzh-d',
  rowKey: 'id'
};

DeviceRegister = Form.create()(DeviceRegister);
...
export default connect(mapStateToProps)(DeviceRegister);
