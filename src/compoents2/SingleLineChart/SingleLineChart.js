...
class SingleLineChart extends Component {
  ...
  renderChart(props = {}) {
    const { data = [], height, title, ...rest } = props;
    ...
    if (!this.chart){
      ...
      //------------------- 轴向别名 ---------------------------------
        ...
        chart.point().position('x*y').size(5).shape('circle').style({
          stroke: '#fff',
          lineWidth: 1
        });
        chart.render();
  ...
  render() {
    ...
    return (
        <div className={cx(`${prefix}-${cssPrefix}`, className)}
          ref={(lineChart) => {this.node = lineChart }}
          {..._props}
          style={_style}
        >
