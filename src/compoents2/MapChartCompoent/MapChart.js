...
class MapChart extends Component {
  ...
  renderChart(props = {}) {
    ...
    if (!this.chart) {
       ...
       *  一。请求要渲染的地图数据。
       * */
      fetch('/api/fake_chart_data/world_geo_data', {
        method: 'GET',
        // body: {},
        headers: {},
      }).then( (response) => {
        console.log('1.请求渲染的地图数据', response);
        let map = [];
        ...
        //----------- 二。请求气泡的位置数据 -------------------------
        fetch('/api/fake_chart_data/china_pm_data', {
          method: 'GET',
          // body: {},
          headers: {},
        }).then((response) => {
          ...
          // Step 3：创建图形语法，绘制柱状图，由 long 和 lant 两个属性决定图形位置，long 映射至 经度，lant 映射至 维度。
