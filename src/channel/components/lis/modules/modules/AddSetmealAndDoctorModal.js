 ...
 * 选项目对话框
 */
class AddSetmealAndDoctorModal extends Component{
  _retInitialState = ()=>{
    return {
  ...
  //  this.getProjectIdsList({pageSize, pageSize});
  //}
  ...
  getProjectIdsList = (query)=>{
    ...
    fetch('doctorAdvice/list',{
      ...
      error: (err)=>{
        message.error(err.message+'，医嘱分页接口报错');
      },
      beforeSend: ()=>{
        this.setState({
          tableLoading: true
    ...
    return (
      <div>
        ...
        >
          ...
          <div style={{color:'#f00',textAlign:'left',}}>(*请谨慎选择医嘱)</div>
          <Table
...
AddSetmealAndDoctorModal.propTypes = {
  data: PropTypes.any,
...
AddSetmealAndDoctorModal.defaultProps = {
  prefix: 'yzh',
  rowKey: 'id',
  visible: false,
  callback: noop,
...
AddSetmealAndDoctorModal = Form.create()(AddSetmealAndDoctorModal);

export default AddSetmealAndDoctorModal;
