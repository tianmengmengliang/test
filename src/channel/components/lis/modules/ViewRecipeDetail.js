...
//--------- start 查询ProjectIds分页列表接口 --------------------------
  ...
  getProjectIdsList = (query)=>{
    ...
    fetch('doctorAdvice/listByRecipeId',{ //套餐id,查医嘱分页。
      // method: 'post',
      // headers: {},
      data: _q,
      success: (res)=>{
...
//----------- end列表 --------------------------------------------------
  ...
  componentWillReceiveProps(nextProps){
    ...
    if(visible && newData !== oldData ){
      if(!!newData.id){
        ...
        // case2. 查询数据。
        const {listPage: {pageIndex, pageSize}} = this.state;
        this.getProjectIdsList({pageIndex, pageSize, recipeId: id}, (listPage)=>{
          this.setState({
            listPage
          })
        });
  ...
  // step1 点击查询按钮查询
  queryClick = ()=>{ //点击查询按钮。
    const {listPage: {pageSize}, RecipeId} = this.state;
    this.getProjectIdsList({pageIndex: 1, pageSize, recipeId: RecipeId,});
  };
  resetClick = ()=>{ //重置查询条件。
    this.props.form.resetFields();
    const {listPage: {pageSize}, RecipeId} = this.state;
  ...
  render(){
    ...
    const {confirmLoading, listPage: {pageIndex, pageSize, rows:dataSource, total}, tableLoading,
      spinning, tip, selectedIds, selectedRows, selectedRowKeys, RecipeId,} = this.state;
    ...
    const _pagination = {
      ...
      onChange: (current) => { //current 鼠标点击的第几页。
        ...
        this.getProjectIdsList({pageIndex: current, pageSize, recipeId: RecipeId,})//分页列表查询。
      }
    };
    return (
      <div>
...
ViewRecipeDetail = Form.create()(ViewRecipeDetail);
