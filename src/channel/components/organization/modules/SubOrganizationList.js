...
class SubOrganizationList extends Component{
  _retInitialState= ()=>{
    return {
  ...
  add = ({type,  data: selectedRows = []})=>{
    ...
    if(type === 'add') {
      ...
      this.getResidencesData((err, res)=> { //1、获取省市区
        ...
        let _dataSource = fn(_data, 0);  //---------- 省市区Json数据---> 树状结构 ------------//树形结构遍历
        /************************************/
        this.showAddPOCTBox({
          data: {
            residences: _dataSource, //省市区
    ...
    if(type === 'edit') {
      ...
      if (hasSelectedOnlyOneIdx) {
        ...
        this.getResidencesData((err, res)=> { //1、获取省市区
          ...
          let _dataSource = fn(_data, 0);  //---------- 省市区Json数据---> 树状结构 ------------//树形结构遍历
          /************************************/
          this.showAddPOCTBox({data: {...row,  residences: _dataSource, parentId: parentId}});
        });
      }else{
        return;
      }
...
//------------------------------------end 删除订单 -----------------------------------------
  ...
  render(){
    ...
    const {confirmLoading, listPage: {pageIndex, pageSize, rows: dataSource, total}, tableLoading, spinning,parentId,
      selectedIds, selectedRows, tip, addSubOrgModal,  } = this.state;
      ...
      <div>
        ...
        >
          <div className="hidden">
            <AddSubOrgModal {...addSubOrgModal} callback={this.addPOCTBoxCallback} />
          </div>
          ...
          <Spin
            ...
            {/* 1.查询栏 */}
            <ButtonContainer>
              <Button onClick={this.add.bind(this, {type: 'add', data:{}})} type="primary"><Icon type="plus" />添加</Button>
...
SubOrganizationList.defaultProps = {
  prefix: 'yzh',
  rowKey: 'id',
  visible: false,
  callback: noop,
...
SubOrganizationList = Form.create()(SubOrganizationList);
