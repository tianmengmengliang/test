...
class AddOrgModal extends Component{
    ...
    this.props.form.validateFieldsAndScroll((err, data) => { //数据。
      ...
      arrSample.map((key, index) => {
        ...
        ['type','code','attchFile','beginDate','endDate'].map((item)=>{
          if(item === 'attchFile'){
            /*********** 上传的文件取路径。****************/
            let _response = key[item][0].response;
            if(_response !== undefined){
              row.attchFile = _response.responseObject; //文件包裹两层
            }
            /********************************************/
          }
  ...
  normFile = (e) => {
    ...
    if(fileList.length > 0){
      let _response = fileList[0].response; //只取一个文件。
      if(_response !== undefined){
          if(_response.success){
            return _response.responseObject;
          }else{
            message.error( _response.msg+',上传失败。'||'上传失败。');
          }
      }
    }*/
  }
  ...
  render(){
    ...
    const {confirmLoading, orgTypleFlag, } = this.state;
        ...
        <td>
          <Form.Item key={k}>
            {getFieldDecorator(`attchFile${index}`,{
              ...
              <Upload name="uploadPassport" action={`${CONFIG.url}passport/upload`}>
                <Button>
                  <Icon type="upload" /> 上传
                </Button>
              </Upload>
          ...
          >
            {getFieldDecorator('type',{
              ...
              <RadioGroup onChange={this.onRadioClick}>
                <RadioButton value={1}>集团</RadioButton>
...
AddOrgModal = Form.create()(AddOrgModal);
