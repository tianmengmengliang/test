...
//--------- start 查询 ProjectIds分页列表接口 --------------------------
  ...
  getProjectIdsList = (query)=>{
    ...
    fetch('workGroupDetail/list',{
      ...
      error: (err)=>{
        message.error(err.message||'列表接口报错');
      },
      beforeSend: ()=>{
        this.setState({
          tableLoading: true
...
//--------------------------------end列表----------------------
  ...
  componentWillReceiveProps(nextProps){
    ...
    if(visible && newData !== oldData) {
      ...
      this.setState({
        workGroupId: newData.id,  //工作组id
      })
      const {listPage: {pageIndex, pageSize}} = this.state;
      this.getProjectIdsList({pageIndex, pageSize, workGroupId:newData.id}, (listPage)=>{
        this.setState({
          listPage
        })
      });
  ...
  //step1 点击查询按钮查询
  queryClick = ()=>{ //点击查询按钮。
    const {listPage: {pageSize}, workGroupId} = this.state;
    this.getProjectIdsList({pageIndex: 1, pageSize, workGroupId});
  };
  resetClick = ()=>{ //重置查询条件。
    this.props.form.resetFields();
    const {listPage: {pageSize}, workGroupId} = this.state;
  ...
  render(){
    ...
    const {confirmLoading, listPage: {pageIndex, pageSize, rows:dataSource, total}, workGroupId,tableLoading,
    ...
    const _pagination = {
      ...
      onChange: (current) => { //current 鼠标点击的第几页。
        ...
        this.getProjectIdsList({pageIndex: current, pageSize, workGroupId})//分页列表查询。
      }
    };
    const rowSelection = {
...
ViewDetailModal = Form.create()(ViewDetailModal);
