...
class ViewDeviceProjectModal extends Component{
    _retInitialState = ()=>{
        return {
...
//--------- start 查询ProjectIds分页列表接口 --------------------------
    ...
    getProjectIdsList = (query)=>{
        ...
        fetch('deviceType/listByDeviceTypeId',{
            ...
            error: (err)=>{
                message.error(err.message +',项目详细列表错误');
            },
            beforeSend: ()=>{
                this.setState({
                    tableLoading: true
    ...
    componentWillReceiveProps(nextProps){
        ...
        if(visible && newData !== oldData ){
            if(!!newData.id){
                ...
                //case2. 查询数据。
                const {listPage: {pageIndex, pageSize}} = this.state;
                this.getProjectIdsList({pageIndex, pageSize, deviceTypeId: id}, (listPage)=>{
                    this.setState({
                        listPage
                    })
                });
    ...
    // step1 点击查询按钮查询
    queryClick = ()=>{ //点击查询按钮。
        const {listPage: {pageSize}, deviceTypeId} = this.state;
        this.getProjectIdsList({pageIndex: 1, pageSize, deviceTypeId,});
    };
    resetClick = ()=>{ //重置查询条件。
        this.props.form.resetFields();
        const {listPage: {pageSize}, deviceTypeId} = this.state;
    ...
    render(){
        ...
        const {confirmLoading, listPage: {pageIndex, pageSize, rows:dataSource, total}, tableLoading,
            spinning, tip, selectedIds, selectedRows, selectedRowKeys, deviceTypeId,} = this.state;
        ...
        const _pagination = {
            ...
            onChange: (current) => {
                ...
                this.getProjectIdsList({pageIndex: current, pageSize, deviceTypeId,})//分页列表查询。
            }
        };
...
ViewDeviceProjectModal.propTypes = {
    data: PropTypes.any,
...
ViewDeviceProjectModal.defaultProps = {
    prefix: 'yzh',
    rowKey: 'id',
    visible: false,
    callback: noop,
...
ViewDeviceProjectModal = Form.create()(ViewDeviceProjectModal);

export default ViewDeviceProjectModal;
