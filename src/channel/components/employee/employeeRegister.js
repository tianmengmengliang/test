...
import {AddEmployeeModal, ViewLabProjectModal, } from './modules/'
...
class employeeRegister extends Component {
  ...
  showAdd = ({data})=>{
    this.setState(({addEmployeeModal})=>{
      return {
        addEmployeeModal: {
          ...addEmployeeModal,
          visible: true,
...
//-----------------------------------------------end 删除订单 --------------------------------------
  ...
  viewDetailClick = ({rowdata: selectedRows})=>{
    this.getLabOptionDetails({selectId: selectedRows.id }, (err, res={})=> { //根据id, 查看详情
      if (err){ message.error('请求明细失败'); return }
      const {responseObject}= res
  ...
  render() {
    ...
    const { listPage: {pageIndex, pageSize, rows:dataSource, total}, tableLoading, spinning,
      selectedIds, selectedRows, tip, addEmployeeModal, viewLabOptionModal,activeKey,} = this.state;
    ...
    return (
      ...
      <div style={{marginTop: -20, }}>
        <div className="hidden">
          <AddEmployeeModal {...addEmployeeModal} callback={this.addCallback}/>
...
employeeRegister.defaultProps = {
  prefix: 'yzh-d',
  rowKey: 'id'
};

employeeRegister = Form.create()(employeeRegister);
...
export default connect(mapStateToProps)(employeeRegister);
