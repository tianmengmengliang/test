...
import {AddEmployeeModal, ViewLabProjectModal, AddDoctorAdvice } from './modules/'
 ...
 *
 */
class employeeDispatch extends Component {
  _retInitialState = ()=>{
    return {
...
//-----------------------------------------------end 删除订单 --------------------------------------
  ...
  viewDetailClick = ({rowdata: selectedRows})=>{
    this.getLabOptionDetails({selectId: selectedRows.id }, (err, res={})=> { //根据id, 查看详情
      if (err){ message.error(err.message ||'请求明细失败'); return }
      const {responseObject}= res
      this.showViewDetailModal({data: {...selectedRows, samples:responseObject }});
    });
  };
  showViewDetailModal = ({data})=>{ //显示查看对话框
    this.setState(({viewLabOptionModal})=>{
      return {
        viewLabOptionModal: {
          ...viewLabOptionModal,
          visible: true,
  ...
  render() {
    ...
    const { listPage: {pageIndex, pageSize, rows:dataSource, total}, tableLoading, spinning,
      selectedIds, selectedRows, tip, addDoctorAdvice, viewLabOptionModal,activeKey,} = this.state;
    ...
    const _columns = [
      ...
      },
      {
        ...
        formatter: ({dependentValues,rowIdx, value, column:{key}})=>{
          switch(value){
            case 0 :  return (<span style={{color:'#00EA00'}}>{'普通用户'}</span> );  break;
            case 1 :  return (<span style={{color:'#f00'}}>{'开户用户' }</span>);  break;
      ...
      <PageHeaderLayout>
        <div style={{marginTop: -20, }}>
          <div className="hidden">
            ...
            <ViewLabProjectModal {...viewLabOptionModal} callback={this.viewDetailCallback}/>
          </div>
          ...
          <Card>
            ...
            <Spin
              ...
              tip={tip}>
              <ButtonContainer>
                <Button onClick={this.add.bind(this, {type: 'add', data:{}})}  type="primary"  icon="plus">添加员工</Button>
...
employeeDispatch.propTypes = {
  prefix: PropTypes.string,
  rowKey: PropTypes.string
};
employeeDispatch.defaultProps = {
  prefix: 'yzh-d',
  rowKey: 'id'
};

employeeDispatch = Form.create()(employeeDispatch);
...
export default connect(mapStateToProps)(employeeDispatch);
