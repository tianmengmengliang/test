...
//-----------------------------------------------end 删除订单 --------------------------------------
  ...
  render() {
    ...
    const { listPage: {pageIndex, pageSize, rows:dataSource, total}, tableLoading, spinning,
      selectedIds, selectedRows, tip, addDeviceTypleModal, addLaboratoryModal,addLabAndD5A5Modal, activeKey, residences,} = this.state;
      ...
      <PageHeaderLayout>
        <div style={{marginTop: -20,background:'#fff'}}>
          ...
          <Tabs defaultActiveKey={activeKey} onChange={this.onTabClick} type="card" style={{fontSize: 16,marginLeft:10}}>
              {/* 1.实验室 */}
              <TabPane tab="&emsp;&emsp;添加实验室&emsp;&emsp;"  key="1">
              ...
              {/* 2.实验室和医嘱关系*/}
              <TabPane tab="&emsp;实验室和医嘱关系&emsp;"  key="2">
                <LaboratoryAndD5A5 />
              </TabPane>
          </Tabs>
