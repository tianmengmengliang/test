  ...
  componentWillReceiveProps(nextProps){
    ...
    if(visible && newData !== oldData ){
      ...
      if(!!newData.id){
        let id = {}
        const {listPage: {pageIndex, pageSize}} = this.state;
        this.getProjectIdsList({pageIndex, pageSize, id: newData.id}, (listPage)=>{
          this.setState({
            listPage
          })
        });
 ...
 //-------------------- 表单提交--------------------------------
  ...
  render(){
    ...
    const {confirmLoading, listPage: {pageIndex, pageSize, rows:dataSource, total}, tableLoading,
      spinning, tip, selectedIds, selectedRows, selectedRowKeys,} = this.state;
    ...
    const _pagination = {
      ...
      onChange: (current) => { //current 鼠标点击的第几页。
        ...
        let id = {id: data.id}
        this.getProjectIdsList({pageIndex: current, pageSize, id})//分页列表查询。
      }
    };
    return (
      <div>
