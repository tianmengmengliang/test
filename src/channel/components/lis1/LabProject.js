...
import {LisLoginModal,AddDeviceTypleModal, AddLabProjectModal, ViewLabProjectModal, } from './modules/'
...
class LabProject extends Component {
  ...
  /*  add = ({type,  data: selectedRows = []})=>{
   if(type === 'add') {
   this.getProjectIds((err, res)=> { //2.设备类型
   if (err) {
   message.warn(`查询项目projectIds不存在`);
   return;
   }
  ...
  showAdd = ({data})=>{
    this.setState(({addLabOptionModal})=>{
      return {
        addLabOptionModal: {
          ...addLabOptionModal,
          visible: true,
...
//-----------------------------------------------end 删除订单 --------------------------------------
  ...
  render() {
    ...
    const { listPage: {pageIndex, pageSize, rows:dataSource, total}, tableLoading, spinning,
      selectedIds, selectedRows, tip, addDeviceTypleModal, addLabOptionModal, viewLabOptionModal,lisLoginModal, activeKey,} = this.state;
      ...
      <PageHeaderLayout>
        <div style={{marginTop: -20, }}>
            <div className="hidden">
              ...
              <AddLabProjectModal {...addLabOptionModal} callback={this.addCallback}/>
